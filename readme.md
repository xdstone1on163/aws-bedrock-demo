...
- ✅ **多梯度测试**: 8K/32K/64K/128K/256K/360K上下文
...
# 性能测试MiniMax M2（含超长上下文）
python cli.py --mode performance --model minimax \
  --iterations 5 \
  --context-sizes 8K,32K,128K,256K
```
...
| 超长上下文测试 | `bedrock_stress_test/` | 支持256K-360K上下文测试 |
...
1. **成本控制**：
   - 超长上下文测试（256K/360K）成本较高，建议先小规模测试
...
| MiniMax M2 | `minimax.minimax-m2` | 400K | 超长上下文支持 |
